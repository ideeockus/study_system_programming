...
{
    // Ваш код здесь
  if (l->owner && l->owner->timestamp == ctx->timestamp) return 1;
  if (l->owner == NULL || l->owner->timestamp > ctx->timestamp) {
    // wait
    lock(&l->lock);
    if (l->owner == NULL) l->owner = ctx;
...
{
  while(ctx->locks) {
    unlock(&ctx->locks->lock);
    notify_all(&ctx->locks->cv);
    ctx->locks = ctx->locks->next;
  }
}